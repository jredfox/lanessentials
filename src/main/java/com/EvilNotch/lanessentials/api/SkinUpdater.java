...
public class SkinUpdater {
	...
	{
		if(!EventHandler.noSkins.containsKey(p.getName()) )//&& JavaUtil.isOnline(url))
		{
			EventHandler.noSkins.put(p.getName(),new IntObj(0));
		}
		else
		{
			EventHandler.noSkins.get(p.getName()).integer = 0;
		}
	}
